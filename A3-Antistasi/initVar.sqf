...
("true" configClasses (configfile >> "CfgMagazines")) apply {
...
(_allPrimaryWeapons + _allHandGuns + _allLaunchers + _allItems) apply {
...
(groupsCSATmid + [CSATSpecOp] + groupsCSATSquad) apply {
	_x apply {
		_typeX = _x;
...
(groupsNATOmid + [NATOSpecOp] + groupsNATOSquad) apply {
	_x apply {
		_typeX = _x;
...
(weaponsNato + weaponsCSAT) apply {
	(_x call BIS_fnc_compatibleItems) apply	{
		_item = _x;
...
if (worldName == "Tanoa") then {
	...
	if (worldName == "Altis") then {
		...
		roadsMrk apply {_x setMarkerAlpha 0;};
...
SDKMil apply {server setVariable [_x, 50, true];};
(sdkTier1 - SDKMil) apply {server setVariable [_x, 75, true];};
sdkTier2 apply {server setVariable [_x, 100, true];};
sdkTier3 apply {server setVariable [_x, 150, true];};
[staticATOccupants, staticAAOccupants] apply {timer setVariable [_x, 3, true];};
[staticATInvaders, staticAAInvaders] apply {timer setVariable [_x, 6, true];};
vehNATOAPC apply {timer setVariable [_x, 0, true];};
vehCSATAPC apply {timer setVariable [_x, 10, true];};
...
vehNATOTransportPlanes apply {timer setVariable [_x, 1, true];};
(vehNATOTransportHelis - [vehNATOPatrolHeli]) apply {timer setVariable [_x, 1, true];};
vehCSATTransportPlanes apply {timer setVariable [_x, 1, true];};
(vehCSATTransportHelis - [vehCSATPatrolHeli]) apply {timer setVariable [_x, 10, true];};
vehNATOAttackHelis apply {timer setVariable [_x, 0, true];};
vehCSATAttackHelis apply {timer setVariable [_x, 10, true];};
...
[vehSDKLightArmed,vehSDKAT] apply {server setVariable [_x,700,true];};
[SDKMGStatic,vehSDKBoat,vehSDKRepair] apply {server setVariable [_x,400,true];};//400
[SDKMortar,staticATteamPlayer,staticAAteamPlayer] apply {server setVariable [_x,800,true];};
//800
