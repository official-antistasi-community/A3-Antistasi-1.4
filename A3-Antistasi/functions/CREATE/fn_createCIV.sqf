...
if ((daytime < 8) or (daytime > 21)) then {
	_civilianPopulation = round (_civilianPopulation / 4);
	_vehicleLimit = round (_vehicleLimit * 1.5)
};
...
private _locationBoatSpawns = if !(hasIFA) then {
	seaSpawn select {getMarkerPos _x inArea _locationMarker}
	} else {
	[];
};
			...
			if (!isNull _road) then {
				if (count (nearestObjects [_position1,["Car","Truck"],5]) == 0) then {
					...
					_newPatrolVehicle addEventHandler ["HandleDamage",{
						if (((_this select 1) find "wheel" != -1) and (_this select 4 == "") and (!isPlayer driver (_this select 0))) then {
							0;
						} else {
							(_this select 2);
						};
...

{
	...
	deleteVehicle _x;
} forEach _spawnedDrivingCivilians;
