...
for "_timeLeft" from _timeLeft to _timeLeft-4 step -1 do {
	...
	["FF Notification", format ["Please do not teamkill. Play with the turtles for %1 more seconds.<br/>Use Refresh Admin Action if the admin just logged in.",_timeLeft], true] remoteExec ["A3A_fnc_customHint", _detainee, false];
	uiSleep 1;
};
true;