...
if (isClass (configFile/"CfgVehicles"/_fuelTank # 0)) then {
    ...
    vehicleBox addAction [format["Buy %1 for %2€",_dispName, _fuelTank # 1], {[player, _this # 3 # 0, _this # 3 # 1, [['A3A_fnc_logistics_addLoadAction', false]]] call A3A_fnc_buyItem},_fuelTank,0,false,true,"","(_this == theBoss)",4];
};
