...
switch (_type) do {
	...
	case "SUPP": {
		...
		if (count _possibleMarkers == 0) then {
			if (!_silent) then {
				[petros,"globalChat",localize "STR_A3A_mission_LOG_Supplies_petros_far"] remoteExec ["A3A_fnc_commsMP",_requester];
				[petros,"hint",localize "STR_A3A_mission_LOG_Supplies_hint_far", localize "STR_A3A_missions_title"] remoteExec ["A3A_fnc_commsMP",_requester];
			};
		} else {
