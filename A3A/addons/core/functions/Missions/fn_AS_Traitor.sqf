...
//converts datenumber back to date array so that time formats correctly
private _dateLimit = numberToDate [date select 0, _dateLimitNum];

//Converts the time portion of the date array to a string for clarity in hints
private _displayTime = [_dateLimit] call A3A_fnc_dateToTimeString;

private _radiusX = [_markerX] call A3A_fnc_sizeMarker;
private _houses = (nearestObjects [_positionX, ["house"], _radiusX]) select {!((typeOf _x) in bld_remove)};
private _posHouse = [];
...
private _sol1 = [_groupTraitor, FactionGet(occ,"unitBodyguard"), _posSol1, [], 0, "NONE"] call A3A_fnc_createUnit;
private _sol2 = [_groupTraitor, FactionGet(occ,"unitBodyguard"), _posSol2, [], 0, "NONE"] call A3A_fnc_createUnit;
...
private _typeGroup = if (random 10 < tierWar) then {selectRandom FactionGet(occ,"groupsSquads")} else {FactionGet(occ,"groupPoliceSquad")};
...
if ({_traitor knowsAbout _x > 1.4} count ([500,0,_traitor,teamPlayer] call A3A_fnc_distanceUnits) > 0) then {
	...
	private _wp0 = _groupTraitor addWaypoint [_posVeh, 0];
	_wp0 setWaypointType "GETIN";
	private _wp1 = _groupTraitor addWaypoint [_posBase, 1];
...
if (not alive _traitor || traitorIntel) then {
	...
	if(traitorIntel && (alive _traitor)) then {
		...
		private _wp1 = _groupTraitor addWaypoint [_posBase];
		_wp1 setWaypointType "MOVE";
		_wp1 setWaypointBehaviour "CARELESS";
		_wp1 setWaypointSpeed "FULL";
	};
	...
	if (dateToNumber date > _dateLimitNum) then {
		...
		if (isPlayer theBoss) then {
			...
			private _minesFIA = allmines - (detectedMines Occupants) - (detectedMines Invaders);
			if (count _minesFIA > 0) then {
				{
					if (random 100 < 30) then {
						Occupants revealMine _x;
					}
				} forEach _minesFIA;
			};
		};
