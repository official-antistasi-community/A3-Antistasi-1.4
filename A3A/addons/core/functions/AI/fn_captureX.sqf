...
if (_recruiting) then {
	_playerX globalChat [localize "STR_A3A_fn_captureX_recr1"];
    ...
    {
        _modAggro = [1, 30];
		_response = localize "STR_A3A_fn_captureX_recr_yes1";
		_modHR = true;
		_fleeSide = teamPlayer;
	}
	else
    {
		_response =  localize "STR_A3A_fn_recr_no1";
		_modAggro = [0, 0];
	};
}
else {
	_playerX globalChat {localize "STR_A3A_fn_captureX_dismiss"};
	private _responseNum = str[(random 3) + 1];
	_response = localize {"STR_A3A_fn_captureX_libresponse" + _responseNum};
    _modAggro = [-3, 30];
};
