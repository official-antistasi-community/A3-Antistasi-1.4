					...
					If (!(fleeing (leader _grp))) then
					{
						...
						If ((random 100) <= 10) then
						{
							...
							If (_AttackPos select 0 != 0 && _AttackPos select 1 != 0) then
								{
								if (side (leader _grp) != rebelSide) then {[[_attackPos,(side(leader _grp)),"",false],"A3A_fnc_patrolCA"] remoteExec ["A3A_fnc_scheduler",2]};
								};//by Barbolani so AI UPSMon garrisons call QRF
						};
					};
				};
			};
