...
class commander_comm 		{
	...
	class controls {
		...
		class 8slots_R3: A3A_core_BattleMenuRedButton
		{
			...
			w = 0.175015 * safezoneW;
			h = 0.0560125 * safezoneH;
			tooltip = "Manage arsenal non-member limits";	//$STR_antistasi_dialogs_commander_comm_faction_garage_tooltip;
			action = "if (player == theBoss) then {closeDialog 0; createDialog ""ArsenalLimitsDialog""} else {[""Arsenal limits"", ""You're not the Commander!""] call A3A_fnc_customHint}";
		};
		class 8slots_L4: A3A_core_BattleMenuRedButton
		{
			idc = -1;
