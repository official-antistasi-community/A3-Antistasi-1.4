...
if (_typeX == "intelError") then {[_titleX, format [localize "STR_A3A_core_fn_base_commsmp_error",_textX]] call A3A_fnc_customHint;};
...
if (_typeX == "countdown") then
	{
	_textX = format [localize "STR_A3A_core_fn_base_commsmp_remaining",_textX];
	[localize "STR_A3A_core_fn_base_commsmp_countdown", format ["%1",_textX]] call A3A_fnc_customHint;
	};
...
if (_typeX == "tier") then
	...
	playSound "3DEN_notificationDefault";
	//[_textX,0.8,0.5,5,0,0,2] spawn bis_fnc_dynamicText;
	_textX = format ["War Level Changed<br/><br/>Current Level: %1",tierWar]; //localize "STR_A3A_core_fn_base_commsmp_warlvlchange"
	[_textX, [safeZoneX + (0.8 * safeZoneW), (0.2 * safeZoneW)], 0.5, 5, 0, 0, _layer] spawn bis_fnc_dynamicText;
	incomeRep = false;
	[] spawn A3A_fnc_statistics;
	};
