...
if (count _todo < 1) exitWith
	{
	if (count _this == 0) then {[localize "STR_A3A_core_fn_ammo_cargo", format [localize "STR_A3A_core_fn_ammo_empty_vehicle_empty", _vehName]] call A3A_fnc_customHint;};
	if (count _this == 2) then {deleteVehicle _truckX};
	};

if (count _this == 0) then {
	[localize "STR_A3A_core_fn_ammo_cargo", format [localize "STR_A3A_core_fn_ammo_empty_cargo_transferred", _vehName]] call A3A_fnc_customHint;
};
