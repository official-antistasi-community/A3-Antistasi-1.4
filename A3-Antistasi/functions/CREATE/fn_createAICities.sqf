...
private ["_markerX","_groups","_soldiers","_positionX","_num","_dataX","_occupantSupport","_rebelSupport","_esAAF","_params","_frontierX","_array","_countX","_groupX","_dog","_grp","_sideX"];
...
if (_markerX in destroyedSites) then
	...
	if (_sideX == Occupants) then
		{
		_num = round (_num * (_occupantSupport + _rebelSupport)/100);
		...
		{
		_esAAF = false;
		_num = round (_num * (_rebelSupport/100));
		_array = [];
