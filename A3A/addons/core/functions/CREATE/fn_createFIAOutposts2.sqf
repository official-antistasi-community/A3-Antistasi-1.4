...
//if ({alive _x} count units _groupX == 0) then
	...
	if (_isRoad) then
		{
		["TaskFailed", ["", localize "STR_A3A_fn_roadblockFight_lost"]] remoteExec ["BIS_fnc_showNotification", 0];
		}
	else
		{
		["TaskFailed", ["", localize "STR_A3A_fn_crfiaOP2_lostWatchpost"]] remoteExec ["BIS_fnc_showNotification", 0];
		};
	};
