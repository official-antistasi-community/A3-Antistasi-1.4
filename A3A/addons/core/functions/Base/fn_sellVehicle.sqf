...
private _titleStr = localize "STR_A3A_core_fn_base_sellveh_sell";
...
if (isNull _veh) exitWith {[_titleStr, localize "STR_A3A_core_fn_base_sellveh_no_looking"] remoteExecCall ["A3A_fnc_customHint",_player];};

if (_veh distance getMarkerPos respawnTeamPlayer > 50) exitWith {[_titleStr, localize "STR_A3A_core_fn_base_sellveh_no_closer"] remoteExecCall ["A3A_fnc_customHint",_player];};

if ({isPlayer _x} count crew _veh > 0) exitWith {[_titleStr, localize "STR_A3A_core_fn_base_sellveh_no_empty"] remoteExecCall ["A3A_fnc_customHint",_player];};
...
if !(_owner isEqualTo "" || {getPlayerUID _player isEqualTo _owner}) exitWith {  // Vehicle cannot be sold if owned by another player.
    [_titleStr, localize "STR_A3A_core_fn_base_sellveh_no_owner"] remoteExecCall ["A3A_fnc_customHint",_player];
};

if (_veh getVariable ["A3A_sellVehicle_inProgress",false]) exitWith {[_titleStr, localize "STR_A3A_core_fn_base_sellveh_progress"] remoteExecCall ["A3A_fnc_customHint",_player];};
...
if (_costs == 0) exitWith {
    ...
    [_titleStr, localize "STR_A3A_core_fn_base_sellveh_no_suitable"] remoteExecCall ["A3A_fnc_customHint",_player];
};
...
[_titleStr, localize "STR_A3A_core_fn_base_sellveh_sold"] remoteExecCall ["A3A_fnc_customHint",_player];
nil;
