...
if (isNull _newBoss) exitWith {
	[_silent] spawn {
		params ["_silent"];
		sleep 5;
		private _textX = format [localize "STR_A3A_fn_tBTransfer_noEligible"];
		if (!_silent) then {[petros,"hint",_textX, localize "STR_A3A_fn_tBTransfer_newCommTitle"] remoteExec ["A3A_fnc_commsMP", 0]};
...
[_silent] spawn {
	params ["_silent"];
	sleep 5;
	private _textX = format [localize "STR_A3A_fn_tBTransfer_newCommLong", name theBoss];
	if (!_silent) then {[petros,"hint",_textX, localize "STR_A3A_fn_tBTransfer_newCommTitle"] remoteExec ["A3A_fnc_commsMP", 0]};
	[] remoteExec ["A3A_fnc_statistics",[teamPlayer,civilian]];
};
