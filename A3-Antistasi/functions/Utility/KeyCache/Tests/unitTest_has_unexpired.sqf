...
private _fnc_reporter = {
    ...
    ["UnitTest KeyCache-Has Unexpired", _text] call A3A_fnc_customHint;
    Info("UnitTest, KeyCache Has Unexpired, " + _text);
};
...
Dev_testHandle = [_fnc_reporter,_reporterContext] spawn {
    ...
    [_reporterContext, "Has Unexpired<br/>Test Started"] call _fnc_reporter;

    //// Assert
    private _passedTest = "SomeTestVariable" call A3A_fnc_keyCache_has && !("SomeUnknownVariable" call A3A_fnc_keyCache_has);
    if (_passedTest) then {
        [_reporterContext, "Has Unexpired<br/>Test Passed"] call _fnc_reporter;
    } else {
        [_reporterContext, "Has Unexpired<br/>Test Failed"] call _fnc_reporter;
    };
