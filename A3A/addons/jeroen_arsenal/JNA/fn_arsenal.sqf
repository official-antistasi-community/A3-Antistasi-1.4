...
switch _mode do {
	...
	case "customEvents":{
		...
		_ctrlButtonImport ctrlSetText localize "STR_antistasi_logistics_JNA_arsenal_text_defaultgear";
		_ctrlButtonImport ctrlSetTooltip localize "STR_antistasi_logistics_JNA_arsenal_tooltip_defaultgear";
		...
		_ctrlButtonRandom ctrlSetText localize "STR_antistasi_logistics_JNA_arsenal_text_tocrate";
		_ctrlButtonRandom ctrlSetTooltip localize "STR_antistasi_logistics_JNA_arsenal_tooltip_tocrate";
      ...
      if (lbSize _ctrlSort > 1) then {
        _ctrlSort lbDelete 1;
			...
			private _sortByAmountIndex =  _ctrlSort lbadd localize "STR_antistasi_logistics_JNA_arsenal_sort_amount";
      private _sortDefaultIndex = _ctrlSort lbadd localize "STR_antistasi_logistics_JNA_arsenal_sort_default";
  ...
  case "SortBy":{
		...
		if _type then{
			...
			if(_index in [
				...
				_strAmount = switch true do {
					case (_amount == 0): {
						localize "STR_antistasi_logistics_JNA_arsenal_assetamount_amount0"
					};
					case (_amount > 50): {
						localize "STR_antistasi_logistics_JNA_arsenal_assetamount_amountmore50"
					};
					case (_amount > 10): {
						localize "STR_antistasi_logistics_JNA_arsenal_assetamount_amountmore10"
					};
					case (_amount > 3): {
						localize "STR_antistasi_logistics_JNA_arsenal_assetamount_amountmore3"
					};
					case (_amount > 1): {
						localize "STR_antistasi_logistics_JNA_arsenal_assetamount_amountmore1"
					};
					case (_amount == 1): {
						localize "STR_antistasi_logistics_JNA_arsenal_assetamount_amount1"
					};
					case (_amount == -1): {//TODO marker for changed entry
						localize "STR_antistasi_logistics_JNA_arsenal_assetamount_amountunlimited"
					};
					default{""};
				};
				...
				_strAmmo = switch true do {
					case (_colorMult == 0): {
						localize "STR_antistasi_logistics_JNA_arsenal_ammoamount_amount0"
					};
					case (_colorMult > 0.9): {
						localize "STR_antistasi_logistics_JNA_arsenal_ammoamount_amountmore09"
					};
					case (_colorMult > 0.2): {
						localize "STR_antistasi_logistics_JNA_arsenal_ammoamount_amountmore02"
					};
					case (_colorMult > 0): {
						localize "STR_antistasi_logistics_JNA_arsenal_ammoamount_amountmore0"
					};
					default{""};
				};
	...
	case "SelectItem": {
		...
		if ((_amount <= _min) AND (_amount != -1) AND (_item !="") AND !([player] call A3A_fnc_isMember) AND !_type) exitWith{
			['showMessage',[_display,localize "STR_antistasi_logistics_JNA_arsenal_weapon_low"]] call jn_fnc_arsenal;
						...
						if (count _magazines > 0) then {
							...
							if([jna_dataList select IDC_RSCDISPLAYARSENAL_TAB_CARGOMAGALL, _mag] call jn_fnc_arsenal_itemCount > 0)then{
								if((player canAddItemToUniform _mag)||(player canAddItemToVest _mag)||(player canAddItemToBackpack _mag))then{
									...
									titleText[localize "STR_antistasi_logistics_JNA_arsenal_batteries_no_space", "PLAIN"];
								};
							}else{
								titleText[localize "STR_antistasi_logistics_JNA_arsenal_batteries_no_left", "PLAIN"];
							};
						};
						[_index, _item]call jn_fnc_arsenal_removeItem;
					};
