	...
	if (_typeX in vehPlanes) then
		...
		if (_veh isKindOf "StaticWeapon") then
			...
			if ((not (_veh in staticsToSave)) and (side gunner _veh != teamPlayer)) then
				{
				if (hasGREF and ((_typeX == staticATteamPlayer) or (_typeX == staticAAteamPlayer))) then {[_veh,"moveS"] remoteExec ["A3A_fnc_flagaction",[teamPlayer,civilian],_veh]} else {[_veh,"steal"] remoteExec ["A3A_fnc_flagaction",[teamPlayer,civilian],_veh]};
				};
