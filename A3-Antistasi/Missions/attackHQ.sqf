...
[[rebelSide,civilian],"DEF_HQ",[format ["Enemy knows our HQ coordinates. They have sent a SpecOp Squad in order to kill %1. Intercept them and kill them. Or you may move our HQ 1Km away so they will loose track",name petros],format ["Defend %1",name petros],rebelRespawn],_positionX,true,10,true,"Defend",true] call BIS_fnc_taskCreate;
[[_sideX],"DEF_HQ1",[format ["We know %2 HQ coordinates. We have sent a SpecOp Squad in order to kill his leader %1. Help the SpecOp team",name petros, rebelFactionName],format ["Kill %1",name petros],rebelRespawn],_positionX,true,10,true,"Attack",true] call BIS_fnc_taskCreate;
...
waitUntil {sleep 1;({[_x] call A3A_fnc_canFight} count _soldiers < {!([_x] call A3A_fnc_canFight)} count _soldiers) or (_positionX distance getMarkerPos rebelRespawn > 999) or (!alive petros)};

if (!alive petros) then
	{
	["DEF_HQ",[format ["Enemy knows our HQ coordinates. They have sent a SpecOp Squad in order to kill %1. Intercept them and kill them. Or you may move our HQ 1Km away so they will loose track",name petros],format ["Defend %1",name petros],rebelRespawn],_positionX,"FAILED"] call A3A_fnc_taskUpdate;
	["DEF_HQ1",[format ["We know %2 HQ coordinates. We have sent a SpecOp Squad in order to kill his leader %1. Help the SpecOp team",name petros,rebelFactionName],format ["Kill %1",name petros],rebelRespawn],_positionX,"SUCCEEDED"] call A3A_fnc_taskUpdate;
	}
else
	{
	if (_positionX distance getMarkerPos rebelRespawn > 999) then
		{
		["DEF_HQ",[format ["Enemy knows our HQ coordinates. They have sent a SpecOp Squad in order to kill Maru. Intercept them and kill them. Or you may move our HQ 1Km away so they will loose track",name petros],format ["Defend %1",name petros],rebelRespawn],_positionX,"SUCCEEDED"] call A3A_fnc_taskUpdate;
		["DEF_HQ1",[_sideX],[format ["We know %2 HQ coordinates. We have sent a SpecOp Squad in order to kill his leader %1. Help the SpecOp team",name petros,rebelFactionName],format ["Kill %1",name petros],rebelRespawn],_positionX,"FAILED"] call A3A_fnc_taskUpdate;
		}
	else
		{
		["DEF_HQ",[format ["Enemy knows our HQ coordinates. They have sent a SpecOp Squad in order to kill %1. Intercept them and kill them. Or you may move our HQ 1Km away so they will loose track",name petros],format ["Defend %1",name petros],rebelRespawn],_positionX,"SUCCEEDED"] call A3A_fnc_taskUpdate;
		["DEF_HQ1",[format ["We know %2 HQ coordinates. We have sent a SpecOp Squad in order to kill his leader %1. Help the SpecOp team",name petros,rebelFactionName],format ["Kill %1",name petros],rebelRespawn],_positionX,"FAILED"] call A3A_fnc_taskUpdate;
		[0,3] remoteExec ["A3A_fnc_prestige",2];
		[0,300] remoteExec ["A3A_fnc_rebelResources",2];
...
if (count _pilots > 0) then
	...
	{
	_veh = _x;
	waitUntil {sleep 1; !([distanceSPWN,1,_x,rebelSide] call A3A_fnc_distanceUnits) and (({_x distance _veh <= distanceSPWN} count (allPlayers - (entities "HeadlessClient_F"))) == 0)};
	deleteVehicle _veh;
	} forEach _pilots;
	};
