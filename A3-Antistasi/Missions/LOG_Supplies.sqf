...
[[rebelSide,civilian],"LOG",[_taskDescription,"City Supplies",_markerX],_positionX,false,0,true,"Heal",true] call BIS_fnc_taskCreate;
missionsX pushBack ["LOG","CREATED"]; publicVariable "missionsX";
_pos = (getMarkerPos rebelRespawn) findEmptyPosition [1,50,"C_Van_01_box_F"];
...
if ((dateToNumber date > _dateLimitNum) or (isNull _truckX)) then
	...
	["TaskFailed", ["", format ["%2 deploying supplies in %1",_nameDest,rebelFactionName]]] remoteExec ["BIS_fnc_showNotification",Occupants];
	{_friendX = _x;
	...
	} forEach ([300,0,_truckX,rebelSide] call A3A_fnc_distanceUnits);
	while {(_countX > 0)/* or (_truckX distance _positionX < 40)*/ and (dateToNumber date < _dateLimitNum) and !(isNull _truckX)} do
		{
		while {(_countX > 0) and (_truckX distance _positionX < 40) and ({[_x] call A3A_fnc_canFight} count ([80,0,_truckX,rebelSide] call A3A_fnc_distanceUnits) == count ([80,0,_truckX,rebelSide] call A3A_fnc_distanceUnits)) and ({(side _x == Occupants) and (_x distance _truckX < 50)} count allUnits == 0) and (dateToNumber date < _dateLimitNum) and (isNull attachedTo _truckX)} do
			{
			_formatX = format ["%1", _countX];
			{if (isPlayer _x) then {[petros,"countdown",_formatX] remoteExec ["A3A_fnc_commsMP",_x]}} forEach ([80,0,_truckX,rebelSide] call A3A_fnc_distanceUnits);
			sleep 1;
			_countX = _countX - 1;
			};
		if (_countX > 0) then
			{
			_countX = 120*_bonus;//120
			if (((_truckX distance _positionX > 40) or (not([80,1,_truckX,rebelSide] call A3A_fnc_distanceUnits)) or ({(side _x == Occupants) and (_x distance _truckX < 50)} count allUnits != 0)) and (alive _truckX)) then {{[petros,"hint","Don't get the truck far from the city center, and stay close to it, and clean all BLUFOR presence in the surroundings or count will restart"] remoteExec ["A3A_fnc_commsMP",_x]} forEach ([100,0,_truckX,rebelSide] call A3A_fnc_distanceUnits)};
			waitUntil {sleep 1; ((_truckX distance _positionX < 40) and ([80,1,_truckX,rebelSide] call A3A_fnc_distanceUnits) and ({(side _x == Occupants) and (_x distance _truckX < 50)} count allUnits == 0)) or (dateToNumber date > _dateLimitNum) or (isNull _truckX)};
			};
		if (_countX < 1) exitWith {};
		};
		if ((dateToNumber date < _dateLimitNum) and !(isNull _truckX)) then
			{
			[petros,"hint","Supplies Delivered"] remoteExec ["A3A_fnc_commsMP",[rebelSide,civilian]];
			...
			if (!isMultiplayer) then {_bonus = _bonus + ((20-rebelTrainingLevel)*0.1)};
			[-1*(20-rebelTrainingLevel),15*_bonus,_markerX] remoteExec ["A3A_fnc_citySupportChange",2];
...
waitUntil {sleep 1; (not([distanceSPWN,1,_truckX,rebelSide] call A3A_fnc_distanceUnits)) or (_truckX distance (getMarkerPos rebelRespawn) < 60)};
