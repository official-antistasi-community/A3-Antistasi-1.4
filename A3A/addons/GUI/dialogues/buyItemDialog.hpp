...
class A3A_BuyItemDialog : A3A_DefaultDialog
{
  idd = A3A_IDD_BUYITEMDIALOG;
  onLoad = "[""onLoad""] spawn A3A_fnc_buyItemDialog";
  ...
  class Controls
  {
    ...
    {
      idc = -1;
      text = $STR_antistasi_dialogs_buy_item_titlebar;
      x = DIALOG_X;
      y = DIALOG_Y - 5 * GRID_H;
      w = DIALOG_W * GRID_W;
      h = 5 * GRID_H;
    ...
    {
      idc = A3A_IDC_BUYITEMMAIN;

      class Controls
      {
        class ItemControlsGroup : A3A_ControlsGroupNoHScrollbars
        {
          idc = A3A_IDC_ITEMGROUP;
          x = 0;
          y = 4 * GRID_H;
          w = PX_W(DIALOG_W);
  ...
  class Objects
  {
    class ItemPreview
    {
      idc = A3A_IDC_BUYITEMOBJECTRENDER;
