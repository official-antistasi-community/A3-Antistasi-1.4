...
private _timeLimit = if (_this get "isDifficult") then {15} else {30};
...
_this set ["Constructor", { // Type: code | Required | Constructor to run at start of task
    private _faction = _this get "faction";
    private _group = createGroup (_this get "side");
    ...
    if (_this get "isDifficult") then {_guardsTypes append [
    ...
    _this set ["official", _official];
    _this set ["guards", _guards];
}];
...
_this set ["Destructor", { // Type: code | Optional | Destructor to run at end of task
    ...
    { deleteVehicle _x } forEach ([_this get "official"] + (_this get "guards"));
}];

_stages = [
    createHashMapFromArray [
        ["Action", { //Type: code | Required | Action to be done in that stage
            private _official = _this get "official";
