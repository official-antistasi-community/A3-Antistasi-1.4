...
if ((count weaponCargo boxX >0) or (count magazineCargo boxX >0) or (count itemCargo boxX >0) or (count backpackCargo boxX >0)) exitWith {[localize "STR_antistasi_journal_entry_header_commander_5", localize "STR_antistasi_customHint_moveHQ_ammobox"] call A3A_fnc_customHint;};

if !(isNull attachedTo petros) exitWith {[localize "STR_antistasi_journal_entry_header_commander_5", localize "STR_antistasi_customHint_moveHQ_petros"] call A3A_fnc_customHint;};
...
if (count _garrison > 0) then
	...
	if ({(alive _x) and (!captive _x) and ((side _x == Occupants) or (side _x == Invaders)) and (_x distance _positionX < 500)} count allUnits > 0) then
		{
		[localize "STR_antistasi_journal_entry_header_commander_2", localize "STR_antistasi_customHint_moveHQ_garrison"] call A3A_fnc_customHint;
		}
	else
		{
