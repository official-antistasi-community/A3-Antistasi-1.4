...
private _babe_em_vars = _climber getvariable "babe_em_vars";

if !(_babe_em_vars select 2) exitwith {};

private _v1 = (asltoagl(atltoasl(_climber modeltoworld (_climber selectionPosition "Head"))) vectorfromto (positioncameratoworld [0,0,5]));
private _v2 = (_climber modeltoworld [0,0,0]) vectorfromto (_climber modeltoworld [0,0,-5]);
private _cos = _v1 vectorCos _v2;
private _dpos = [0,0,0];
private _pos = [0,0,0];
private _posa = [0,0,0];
private _posb = [0,0,0];
private _toppos = [0,0,0];
private _posWT = [0,0,0];
private _goodZ = [];
	...
	for "_i" from 0 to 20 do { 
		...
		if (_intcount == 0) then {
			...
			switch (currentWeapon _climber) do {
				case (""):
				{
					_animation = "babe_drop_ua";
				};
				case (primaryWeapon _climber):
				{
					_animation = "babe_drop_rfl";
				};
				case (handgunWeapon _climber):
				{
					_animation = "babe_drop_pst";
				};
			};

			if (_animation != "" && str _dpos != "[0,0,0]") then {

				_babe_em_vars = _climber getvariable "babe_em_vars";
				_babe_em_vars set [0, true];
				_climber setVariable ["babe_em_vars", _babe_em_vars];

				[((name _climber) + "EH_em_drop"), {animationState (_condpars select 0) == (_condpars select 1)}, [_climber, _animation], "babe_em_fnc_exec_drop", [_dpos, _climber], true, "babe_em_fnc_finish_drop", [_climber], 0] call babe_core_fnc_addEH;

				[_climber, _animation] remoteExec ["playMove"];
...
private _toptest = lineintersectsSurfaces [agltoasl _posa, agltoasl _posb, _climber, objNull, true, 1, "GEOM", "FIRE"];
...
if (!_top) then {
	_posWT = _climber worldToModel asltoagl _pos;
	private _a = agltoasl (_climber modeltoworld [(_posWT select 0) + 0.3, _posWT select 1, (_posWT select 2)+0.2]);
	private _b = agltoasl (_climber modeltoworld [(_posWT select 0) - 0.3, _posWT select 1, (_posWT select 2)+0.2]);
	private _c = agltoasl (_climber modeltoworld [(_posWT select 0) + 0.3, (_posWT select 1) + 0.1, (_posWT select 2)+0.2]);
	private _d = agltoasl (_climber modeltoworld [(_posWT select 0) - 0.3, (_posWT select 1) - 0.1, (_posWT select 2)+0.2]);
	private _e = agltoasl (_climber modeltoworld [(_posWT select 0) + 0.3, (_posWT select 1) - 0.1, (_posWT select 2)+0.2]);
	private _f = agltoasl (_climber modeltoworld [(_posWT select 0) - 0.3, (_posWT select 1) + 0.1, (_posWT select 2)+0.2]);
	...
	_wide = if (((count _int1) + (count _int2) + (count _int3)) == 0) then {true} else {false};
};
...
if (str _pos != "[0,0,0]" && count _poses > 0) then {
	if (_top) then {
		...
		private _int4 = lineintersectsSurfaces [_posa, _posb, _climber, objNull, true, 1, "GEOM", "FIRE"];
		...
		private _int5 = lineintersectsSurfaces [_posa, _posb, _climber, objNull, true, 1, "GEOM", "FIRE"];
