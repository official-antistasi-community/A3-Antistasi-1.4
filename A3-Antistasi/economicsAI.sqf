...
_fnc_economics = {
	...
	if (_random == "random") then {
		...
		if (_currentItems < _maxItems) then {
			timer setVariable [selectRandom _typeX, _currentItems + _coefficient, true];
		};
	};
};
...
[0.2, "", staticATOccupants, (_outposts * 0.2 + _airbases * 0.5) * _accelerator) * _accelerator] spawn _fnc_economics;
