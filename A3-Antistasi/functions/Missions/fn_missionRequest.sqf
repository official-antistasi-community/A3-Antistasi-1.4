...
if ([_typeX] call BIS_fnc_taskExists) exitWith {if (!_silencio) then {[(call A3A_fnc_getPetros),"globalChat","I already gave you a mission of this type"] remoteExec ["A3A_fnc_commsMP",theBoss]}};
...
if (_typeX == "AS") then
	...
	if (count _potentials == 0) then
		...
		if (!_silencio) then
			{
			[(call A3A_fnc_getPetros),"globalChat","I have no assasination missions for you. Move our HQ closer to the enemy or finish some other assasination missions in order to have better intel"] remoteExec ["A3A_fnc_commsMP",theBoss];
			[(call A3A_fnc_getPetros),"hint","Assasination Missions require cities, Patrolled Jungles or Airports closer than 4Km from your HQ."] remoteExec ["A3A_fnc_commsMP",theBoss];
			};
		}
	else
		{
...
if (_typeX == "CON") then
	...
	if (count _potentials == 0) then
		...
		if (!_silencio) then
			{
			[(call A3A_fnc_getPetros),"globalChat","I have no Conquest missions for you. Move our HQ closer to the enemy or finish some other conquest missions in order to have better intel."] remoteExec ["A3A_fnc_commsMP",theBoss];
			[(call A3A_fnc_getPetros),"hint","Conquest Missions require roadblocks or outposts closer than 4Km from your HQ."] remoteExec ["A3A_fnc_commsMP",theBoss];
			};
		}
	else
		{
...
if (_typeX == "DES") then
	...
	if (count _potentials == 0) then
		...
		if (!_silencio) then
			{
			[(call A3A_fnc_getPetros),"globalChat","I have no destroy missions for you. Move our HQ closer to the enemy or finish some other destroy missions in order to have better intel"] remoteExec ["A3A_fnc_commsMP",theBoss];
			[(call A3A_fnc_getPetros),"hint","Destroy Missions require Airbases or Radio Towers closer than 4Km from your HQ."] remoteExec ["A3A_fnc_commsMP",theBoss];
			};
		}
	else
		{
...
if (_typeX == "LOG") then
	...
	if (count _potentials == 0) then
		...
		if (!_silencio) then
			{
			[(call A3A_fnc_getPetros),"globalChat","I have no logistics missions for you. Move our HQ closer to the enemy or finish some other logistics missions in order to have better intel"] remoteExec ["A3A_fnc_commsMP",theBoss];
			[(call A3A_fnc_getPetros),"hint","Logistics Missions require Outposts, Cities or Banks closer than 4Km from your HQ."] remoteExec ["A3A_fnc_commsMP",theBoss];
			};
		}
	else
		{
...
if (_typeX == "RES") then
	...
	if (count _potentials == 0) then
		...
		if (!_silencio) then
			{
			[(call A3A_fnc_getPetros),"globalChat","I have no rescue missions for you. Move our HQ closer to the enemy or finish some other rescue missions in order to have better intel"] remoteExec ["A3A_fnc_commsMP",theBoss];
			[(call A3A_fnc_getPetros),"hint","Rescue Missions require Cities or Airports closer than 4Km from your HQ."] remoteExec ["A3A_fnc_commsMP",theBoss];
			};
		}
	else
		{
...
if (_typeX == "CONVOY") then
	...
	if (!bigAttackInProgress) then
		...
		if (count _potentials == 0) then
			...
			if (!_silencio) then
				{
				[(call A3A_fnc_getPetros),"globalChat","I have no Convoy missions for you. Move our HQ closer to the enemy or finish some other missions in order to have better intel"] remoteExec ["A3A_fnc_commsMP",theBoss];
				[(call A3A_fnc_getPetros),"hint","Convoy Missions require Airports or Cities closer than 5Km from your HQ, and they must have an idle friendly base in their surroundings."] remoteExec ["A3A_fnc_commsMP",theBoss];
				};
			}
		else
			{
		...
		{
		[(call A3A_fnc_getPetros),"globalChat","There is a big battle around, I don't think the enemy will send any convoy"] remoteExec ["A3A_fnc_commsMP",theBoss];
		[(call A3A_fnc_getPetros),"hint","Convoy Missions require a calmed status around the island, and now it is not the proper time."] remoteExec ["A3A_fnc_commsMP",theBoss];
		};
	};

if ((count _potentials > 0) and (!_silencio)) then {[(call A3A_fnc_getPetros),"globalChat","I have a mission for you"] remoteExec ["A3A_fnc_commsMP",theBoss]};
