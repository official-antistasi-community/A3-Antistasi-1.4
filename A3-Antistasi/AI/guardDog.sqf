...
while {alive _dog} do
	...
	if (isNull _spotted) then
		...
		if ((random 10 < 1) and (isNull _spotted)) then
			{
			playSound3D [missionPath + (selectRandom dogSounds),_dog, false, getPosASL _dog, 1, 1, 100];
			};
		...
		playSound3D [missionPath + (dogSounds select (floor random 5)),_dog, false, getPosASL _dog, 1, 1, 100];
