...
private _titleStr = localize "STR_A3A_core_fn_base_placeselec_title";
...
// localize "STR_A3A_core_fn_base_placeselec_petros_dead" - implement into line 8
// localize "STR_A3A_core_fn_base_placeselec_petros_dead_long" - implement into line 8
...
while {_positionIsInvalid} do {
	...
	if (getMarkerPos _markerX distance _positionClicked < 500) then {
		[_titleStr, localize "STR_A3A_core_fn_base_placeselec_no_enemy_zone"] call A3A_fnc_customHint;
		_positionIsInvalid = true;
	};

	if (!_positionIsInvalid && {surfaceIsWater _positionClicked}) then {
		[_titleStr, localize "STR_A3A_core_fn_base_placeselec_no_water"] call A3A_fnc_customHint;
		_positionIsInvalid = true;
	};

	if (!_positionIsInvalid && (_positionClicked findIf { (_x < 0) || (_x > worldSize)} != -1)) then {
		[_titleStr, localize "STR_A3A_core_fn_base_placeselec_no_map"] call A3A_fnc_customHint;
		_positionIsInvalid = true;
	};
	...
	if (!_positionIsInvalid) then {
		...
		if (_positionIsInvalid) then {[_titleStr, localize "STR_A3A_core_fn_base_placeselec_no_enemy_near"] call A3A_fnc_customHint;};
	};
	sleep 0.1;
};
