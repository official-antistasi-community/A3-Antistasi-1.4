...
if ((_boxX == boxX) and (player!=theBoss)) exitWith {[localize "STR_antistasi_customHint_cargo", localize "STR_antistasi_customHint_cargo_no_comander"] call A3A_fnc_customHint; [driver _truckX,"truckX"] remoteExec ["A3A_fnc_flagaction",driver _truckX]};
...
if (_countX < 1) then
	{
	[localize "STR_antistasi_customHint_cargo", localize "STR_antistasi_customHint_cargo_empty"] call A3A_fnc_customHint;
	_proceed = true;
	};
...
if (_countX > 0) then
	...
	while {(_truckX == vehicle player) and (speed _truckX == 0) and (_countX > 0)} do
		{
		[localize "STR_antistasi_customHint_cargo", format [localize "STR_antistasi_customHint_cargo_loading", _countX]] call A3A_fnc_customHint;
		_countX = _countX -1;
		sleep 1;
		if (_countX == 0) then
			{
		...
		if ((_truckX != vehicle player) or (speed _truckX != 0)) then
				{
				[localize "STR_antistasi_customHint_cargo", localize "STR_antistasi_customHint_cancel"] call A3A_fnc_customHint;
				_proceed = true;
				};
		};
	};
