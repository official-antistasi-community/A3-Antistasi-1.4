...
if (_this select 0 == "add") then
	...
	Info_4("Admin %1 (ID: %2) has added %3 (ID: %4) to the server members list.", name player, getPlayerUID player, name _target, _uid);
	...
	Info_4("Admin %1 (ID: %2) has removed %3 (ID: %4) from the server members list.", name player, getPlayerUID player, name _target, _uid);
