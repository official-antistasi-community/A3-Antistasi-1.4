...
if (_x select 1 >= minWeaps) then
	...
	//if !(_weaponX in mlaunchers) then  // PBP - if we dont allow mlaunchers into this check, why do we look for them later? remove it there if you really want, or make a parameter
		//{
		
		/* PBP - I commented out this section to prevent a weapons unlocking from auto-unlocking its first magazine entry from cfg
