...
private _fnc_doHeal = {
    ...
    if (_cured && _medic != _target && _isPlayerGroup) then {
        _medic groupChat format [localize "STR_A3A_fn_help_ready",name _target];
    };
    _medic stop false;
    _target stop false;
...
if (_medicX != _unit) then
{
    ...
    {
        if (_isPlayerGroup) then {_unit groupChat format [localize "STR_A3A_fn_help_hurt", name _unit]};
    ...
    if (_isPlayerGroup) then {
        _medicX groupChat format [localize "STR_A3A_fn_help_support", name _unit]
    };
