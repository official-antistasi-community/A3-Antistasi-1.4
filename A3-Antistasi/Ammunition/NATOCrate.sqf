...
diag_log format ["%1: [Antistasi] | INFO | NATOCrate | Weapon Types Roll: %2",servertime,(_weaponTypes + 1)];
private _itemTypes = crateItemTypeMin + floor random (crateItemTypeMax - crateItemTypeMin);
diag_log format ["%1: [Antistasi] | INFO | NATOCrate | Item Types Roll: %2",servertime,(_itemTypes + 1)];
private _ammoTypes = crateAmmoTypeMin + floor random (crateAmmoTypeMax - crateAmmoTypeMin);
diag_log format ["%1: [Antistasi] | INFO | NATOCrate | Ammo Types Roll: %2",servertime,(_ammoTypes + 1)];
private _mineTypes = crateMineTypeMin + floor random (crateMineTypeMax - crateMineTypeMin);
diag_log format ["%1: [Antistasi] | INFO | NATOCrate | Mine Types Roll: %2",servertime,(_mineTypes + 1)];
private _opticTypes = crateOpticTypeMin + floor random (crateOpticTypeMax - crateOpticTypeMin);
diag_log format ["%1: [Antistasi] | INFO | NATOCrate | Optic Types Roll: %2",servertime,(_opticTypes + 1)];
...
for "_i" from 0 to _weaponTypes do
	...
	if (isNil "_loot") then
		{
		diag_log format ["%1: [Antistasi] | INFO | NATOCrate | No Weapons Left in Loot List",servertime];
		}
		else
		{
		_num = crateWepNumMin + floor random (crateWepNumMax - crateWepNumMin);
		_crate addWeaponWithAttachmentsCargoGlobal [[ _loot, "", "", "", [], [], ""], _num];
		diag_log format ["%1: [Antistasi] | INFO | NATOCrate | Spawning %2 of %3",servertime,_num, _loot];
		};
	};
...
for "_i" from 0 to _itemTypes do
	...
	if (isNil "_loot") then
		{
		diag_log format ["%1: [Antistasi] | INFO | NATOCrate | No Items Left in Loot List",servertime];
		}
		else
		{
...
for "_i" from 0 to _ammoTypes do
	...
	if (isNil "_loot") then
		{
		diag_log format ["%1: [Antistasi] | INFO | NATOCrate | No Ammo Left in Loot List",servertime];
		}
		else
		{
...
for "_i" from 0 to _mineTypes do
	...
	if (isNil "_loot") then
		{
		diag_log format ["%1: [Antistasi] | INFO | NATOCrate | No Mines Left in Loot List",servertime];
		}
		else
		{
...
if !(hasIFA) then
	...
	for "_i" from 0 to _opticTypes do
		...
		if (isNil "_loot") then
			{
			diag_log format ["%1: [Antistasi] | INFO | NATOCrate | No Optics Left in Loot List",servertime];
			}
			else
			{
	...
	if (round random 100 < 25) then
		...
		diag_log format ["%1: [Antistasi] | INFO | NATOCrate | Spawning 1 Laser Designator",servertime];
		};
		}
	else
		{
	...
	if (hasACE) then
		...
		diag_log format ["%1: [Antistasi] | INFO | NATOCrate | Spawning 1 HuntIR Monitor And 3 Rounds",servertime];
		};
	};