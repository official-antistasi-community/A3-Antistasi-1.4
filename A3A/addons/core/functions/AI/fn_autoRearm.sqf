...
if ((_primaryWeapon in FactionGet(reb,"initialRebelEquipment")) || (_primaryWeapon isEqualTo "")) then {
	...
	if ((_foundItem) && (_unit getVariable "rearming")) then {
		...
		if (_inPlayerGroup) then {_unit groupChat [localize "STR_A3A_fn_autoRearm_getGun"]};
		_timeOut = time + 60;
		...
		if (_unit distance _selectedContainer < 3) then {
			...
			if (primaryWeapon _unit isEqualTo _selectedWeapon) then {
				if (_inPlayerGroup) then {_unit groupChat [localize "STR_A3A_fn_autoRearm_haveGun"]};
...
if ((_foundItem) && (_unit getVariable "rearming")) then {
	...
	if (!((alive _selectedContainer) || (_selectedContainer isKindOf "ReammoBox_F"))) then {_selectedContainer setVariable ["busy",true]};
	_unit doMove (getPosATL _selectedContainer);
	if (_inPlayerGroup) then {_unit groupChat [localize "STR_A3A_fn_autoRearm_rearming"]};
	_timeOut = time + 60;
	waitUntil {sleep 1; !([_unit] call A3A_fnc_canFight) || (isNull _selectedContainer) || (_unit distance _selectedContainer < 3) || (_timeOut < time) || (unitReady _unit)};
	if ((unitReady _unit) && ([_unit] call A3A_fnc_canFight) && (_unit distance _selectedContainer > 3) && (_selectedContainer isKindOf "ReammoBox_F") && (!isNull _selectedContainer)) then {_unit setPos position _selectedContainer};
	if (_unit distance _selectedContainer < 3) then {
		...
		if ({_x in _primaryMagazines} count (magazines _unit) >= _targetMagazines) then {
			if (_inPlayerGroup) then {_unit groupChat [localize "STR_A3A_fn_autoRearm_rearmed"];
		} else {
			if (_inPlayerGroup) then {_unit groupChat [localize "STR_A3A_fn_autoRearm_partRearm"]};
		};
	};
	_selectedContainer setVariable ["busy",false];
} else {
	if (_inPlayerGroup) then {_unit groupChat [localize "STR_A3A_fn_autoRearm_noSourcePrim"]};
};
...
if ((_secondaryWeapon == "") && (loadAbs _unit < 340)) then {
	...
	if ((_foundItem) && (_unit getVariable "rearming")) then {
		...
		if ((!alive _selectedContainer) || (!(_selectedContainer isKindOf "ReammoBox_F"))) then {_selectedContainer setVariable ["busy",true]};
		_unit doMove (getPosATL _selectedContainer);
		if (_inPlayerGroup) then {_unit groupChat [localize "STR_A3A_fn_autoRearm_getSec"]};
		_timeOut = time + 60;
		waitUntil {sleep 1; !([_unit] call A3A_fnc_canFight) || (isNull _selectedContainer) || (_unit distance _selectedContainer < 3) || (_timeOut < time) || (unitReady _unit)};
		if ((unitReady _unit) && ([_unit] call A3A_fnc_canFight) && (_unit distance _selectedContainer > 3) && (_selectedContainer isKindOf "ReammoBox_F") && (!isNull _selectedContainer)) then {_unit setPos position _selectedContainer};
		if (_unit distance _selectedContainer < 3) then {
			...
			if (secondaryWeapon _unit == _selectedWeapon) then {
				if (_inPlayerGroup) then {_unit groupChat [localize "STR_A3A_fn_autoRearm_haveSec"]};
...
if (_secondaryWeapon != "") then {
	...
	if ((_foundItem) && (_unit getVariable "rearming")) then {
		...
		if (_inPlayerGroup) then {_unit groupChat [localize "STR_A3A_fn_autoRearm_noSourceSec"]};
	};
	sleep 3;
};
...
if (!haveRadio && {!(_unit call A3A_fnc_hasARadio)}) then {
	...
	if ((_foundItem) && (_unit getVariable "rearming")) then {
		...
		_unit doMove (getPosATL _selectedContainer);
		if (_inPlayerGroup) then {_unit groupChat [localize "STR_A3A_fn_autoRearm_getRadio"]};
		_timeOut = time + 60;
		waitUntil {sleep 1; !([_unit] call A3A_fnc_canFight) || (isNull _selectedContainer) || (_unit distance _selectedContainer < 3) || (_timeOut < time) || (unitReady _unit)};
		if (_unit distance _selectedContainer < 3) then {
			_unit action ["rearm",_selectedContainer];
...
if (hmd _unit == "") then {
	...
	if ((_foundItem) && (_unit getVariable "rearming")) then {
		...
		_unit doMove (getPosATL _selectedContainer);
		if (_inPlayerGroup) then {_unit groupChat [localize "STR_A3A_fn_autoRearm_getNVGs"]};
		_timeOut = time + 60;
		waitUntil {sleep 1; !([_unit] call A3A_fnc_canFight) || (isNull _selectedContainer) || (_unit distance _selectedContainer < 3) || (_timeOut < time) || (unitReady _unit)};
		if (_unit distance _selectedContainer < 3) then {
			_unit action ["rearm",_selectedContainer];
...
if (!(headgear _unit in allArmoredHeadgear)) then {
	...
	if ((_foundItem) && (_unit getVariable "rearming")) then {
		...
		_unit doMove (getPosATL _selectedContainer);
		if (_inPlayerGroup) then {_unit groupChat [localize "STR_A3A_fn_autoRearm_getHelm"]};
		_timeOut = time + 60;
		waitUntil {sleep 1; !([_unit] call A3A_fnc_canFight) || (isNull _selectedContainer) || (_unit distance _selectedContainer < 3) || (_timeOut < time) || (unitReady _unit)};
		if (_unit distance _selectedContainer < 3) then {
			_unit action ["rearm",_selectedContainer];
...
if ({_x == "FirstAidKit"} count (items _unit) < _targetFAKs) then {
	...
	if ((_foundItem) && (_unit getVariable "rearming")) then {
		...
		_selectedContainer setVariable ["busy",true];
		_unit doMove (getPosATL _selectedContainer);
		if (_inPlayerGroup) then {_unit groupChat [localize "STR_A3A_fn_autoRearm_getFAK"]};
		_timeOut = time + 60;
		waitUntil {sleep 1; !([_unit] call A3A_fnc_canFight) || (isNull _selectedContainer) || (_unit distance _selectedContainer < 3) || (_timeOut < time) || (unitReady _unit)};
		if (_unit distance _selectedContainer < 3) then {
...
if (backpack _unit == "") then {
	...
	if ((_foundItem) && (_unit getVariable "rearming")) then {
		...
		_selectedContainer setVariable ["busy",true];
		_unit doMove (getPosATL _selectedContainer);
		if (_inPlayerGroup) then {_unit groupChat [localize "STR_A3A_fn_autoRearm_getPack"]};
		_timeOut = time + 60;
		waitUntil {sleep 1; !([_unit] call A3A_fnc_canFight) || (isNull _selectedContainer) || (_unit distance _selectedContainer < 3) || (_timeOut < time) || (unitReady _unit)};
		if (_unit distance _selectedContainer < 3) then {
			_unit addBackPackGlobal ((backpack _selectedContainer) call A3A_fnc_basicBackpack);
...
if (!_needsRearm) then {if (_inPlayerGroup) then {_unit groupChat [localize "STR_A3A_fn_autoRearm_noNeed"]}} else {if (_inPlayerGroup) then {_unit groupChat [localize "STR_A3A_fn_autoRearm_done"]}};
